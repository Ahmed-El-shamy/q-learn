...
const MobileSidebar: FC<Omit<NavbarProps, "logoImg">> = ({ links=[], logoText }) => {
    ...
    return (
        <div className="lg:hidden">
            ...
            <aside className={`z-10 bg-white border-l-primary w-2/3 py-4 flex flex-col max-w-lg h-screen overflow-y-auto fixed top-0 left-0 duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <div className="flex justify-between items-center border-b border-b-primary px-6 pb-2">
                    <p className="text-xl">{logoText}</p>
                    <button onClick={toggleMenu} className="cursor-pointer p-1 hover:bg-gray-100 duration-100 rounded">
                        <X size={18} />
                    </button>
                </div>
            ...
            </aside>
            {
                isOpen && (
                    <>
                        <div className="fixed top-0 start-0 duration-100 bg-black/30 h-full w-full" onClick={toggleMenu} />
                    </>
                )
            }
        </div>
